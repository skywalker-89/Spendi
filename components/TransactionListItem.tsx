...
function TransactionInfo({
  ...
  // Adjust the timestamp to UTC manually
  const transactionDate = new Date(date * 1000);

  // Manually format the date with UTC methods
  const formattedDate = `${transactionDate
    .toUTCString()
    .slice(0, 3)} ${transactionDate.getUTCDate()} ${transactionDate
    .toUTCString()
    .slice(8, 11)} ${transactionDate.getUTCFullYear()}`;
